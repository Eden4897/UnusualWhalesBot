...
export async function watchDarkFlow(browser: puppeteer.Browser): Promise<puppeteer.Page> {
...
async function newDarkFlowResponse(info: DarkFlowInfo) {
    ...
    guilds.forEach(async guildInfo => {
        if (!guildInfo.darkFlowChannel) return;
        const guild = await bot.guilds.fetch(guildInfo.id);
        const channel: TextChannel = await guild.channels.fetch(guildInfo.darkFlowChannel) as TextChannel;
