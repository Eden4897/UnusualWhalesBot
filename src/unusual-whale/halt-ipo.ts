...
export async function watchHaltIPO(browser: puppeteer.Browser): Promise<puppeteer.Page> {
...
async function observeNewHaltIPO(page: puppeteer.Page) {
    ...
    if (DEBUG) await page.evaluate(() => document.querySelector('tbody > tr:nth-child(1)').remove())
}
...
async function newHaltIPOResponse(info: HaltIPOInfo) {
    ...
    guilds.forEach(async guildInfo => {
        if (!guildInfo.haltIPOChannel) return;
        const guild = await bot.guilds.fetch(guildInfo.id);
        const channel: TextChannel = await guild.channels.fetch(guildInfo.haltIPOChannel) as TextChannel;
