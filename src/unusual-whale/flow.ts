...
export async function watchFlow(browser: puppeteer.Browser): Promise<puppeteer.Page> {
    ...
    await page.evaluate(async () => {
        const mutationObserver = new MutationObserver(() => {
            ...
            const info: FlowInfo = {
                ...
                Contract: topItem.querySelector('td:nth-child(5)').textContent.toUpperCase(),
        ...
        mutationObserver.observe(
            document.querySelector('#flow-trades > div.os-padding > div > div > table > tbody'),
            { childList: true }
        )
    })

    await new Promise((res) => setTimeout(res, 10000))
    if (DEBUG) await page.evaluate(() => document.querySelector('#flow-trades > div.os-padding > div > div > table > tbody > tr:nth-child(1)').remove())
}

async function newFlowResponse(info: FlowInfo) {
    const embed = new MessageEmbed();
    embed.setTitle(`${info.Ticker} ${info.Contract}`).setColor(info.Side === 'BUY' ? '#39fc03' : '#fc0303')
        .addFields((Object.entries(info)).map((entry) => { return { name: entry[0], value: entry[1] ? entry[1] : 'n/a', inline: true } })).addField('\u200b', '\u200b', true)

    const guilds = guildsFile.read();
    guilds.forEach(async guildInfo => {
        if (!guildInfo.flowChannel) return;
        const guild = await bot.guilds.fetch(guildInfo.id);
        const channel: TextChannel = await guild.channels.fetch(guildInfo.flowChannel) as TextChannel;
