...
export async function watchFlowAlert(browser: puppeteer.Browser): Promise<puppeteer.Page> {
...
async function newFlowAlertResponse(info: FlowAlertInfo) {
    ...
    guilds.forEach(async guildInfo => {
        if (!guildInfo.flowAlertChannel) return;
        const guild = await bot.guilds.fetch(guildInfo.id);
        const channel: TextChannel = await guild.channels.fetch(guildInfo.flowAlertChannel) as TextChannel;
