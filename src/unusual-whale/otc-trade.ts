...
export async function watchOTCTrade(browser: puppeteer.Browser): Promise<puppeteer.Page> {
...
async function newOTCTradeResponse(info: OTCTradeInfo) {
    ...
    guilds.forEach(async guildInfo => {
        if (!guildInfo.OTCTradeChannel) return;
        const guild = await bot.guilds.fetch(guildInfo.id);
        const channel: TextChannel = await guild.channels.fetch(guildInfo.OTCTradeChannel) as TextChannel;
